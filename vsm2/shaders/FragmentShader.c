...
{	
	...
	//gl_FragColor = vec4(1,1,1,1) * (min(float(1),ka + kd * dif))
    gl_FragColor =  vec4(shadow) * shading;// *gl_Color;
    //gl_FragColor = vec4(texture2D(ShadowMap, ShadowCoordPostW.xy).r,0,0,1);
